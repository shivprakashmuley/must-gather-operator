...
func Test_validateSFTPWithRetry(t *testing.T) {
	tests := []struct {
		name           string
		mockDialErrors []error // errors to return on each call, nil means success
		wantErr        bool
		errContains    string
		expectedCalls  int
	}{
		{
			...
			mockDialErrors: []error{nil},
			wantErr:        false,
			expectedCalls:  1,
		},
		{
			name: "success after transient retry",
			mockDialErrors: []error{
				context.DeadlineExceeded, // first attempt - transient
				nil,                      // second attempt - success
			},
			wantErr:       false,
			expectedCalls: 2,
		},
		{
			name: "success after multiple transient retries",
			mockDialErrors: []error{
				context.DeadlineExceeded, // first attempt - transient
				context.DeadlineExceeded, // second attempt - transient
				nil,                      // third attempt - success
			},
			wantErr:       false,
			expectedCalls: 3,
		},
		{
			name: "failure on non-transient error - no retry",
			mockDialErrors: []error{
				errors.New("Authentication failed: username or password is incorrect"),
			},
			wantErr:       true,
			errContains:   "Authentication failed",
			expectedCalls: 1,
		...
		{
			name: "failure after max retries on transient error",
			mockDialErrors: []error{
				context.DeadlineExceeded, // first attempt
				context.DeadlineExceeded, // second attempt
				context.DeadlineExceeded, // third attempt (max)
			},
			wantErr:       true,
			errContains:   "validation timed out after 3 attempts",
			expectedCalls: 3,
		},
		{
			name: "non-transient error after transient - stops retry",
			mockDialErrors: []error{
				context.DeadlineExceeded, // first attempt - transient, retry
				errors.New("Authentication failed: username or password is incorrect"), // second attempt - non-transient, stop
			},
			wantErr:       true,
			errContains:   "Authentication failed",
			expectedCalls: 2,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			sftpDialFunc = func(ctx context.Context, username, password, host string) error {
				...
				// Should not reach here in well-defined tests
			...
			if tt.wantErr {
				if err == nil {
					t.Errorf("validateSFTPWithRetry() expected error, got nil")
				} else if tt.errContains != "" && !strings.Contains(err.Error(), tt.errContains) {
					t.Errorf("validateSFTPWithRetry() error = %v, want error containing %q", err, tt.errContains)
				}
			} else {
				if err != nil {
					t.Errorf("validateSFTPWithRetry() unexpected error = %v", err)
				}
			}

			if callCount != tt.expectedCalls {
				t.Errorf("validateSFTPWithRetry() called dial %d times, expected %d", callCount, tt.expectedCalls)
			}
		})
	}
}
