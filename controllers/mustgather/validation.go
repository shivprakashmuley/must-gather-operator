...
func validateSFTPWithRetry(ctx context.Context, reqLogger logr.Logger, username, password, host string) error {
	...
	for attempt := 1; attempt <= MaxSFTPValidationRetries; attempt++ {
		...
		if attempt < MaxSFTPValidationRetries {
			reqLogger.Info("SFTP validation transient error, retrying",
				"error", lastErr, "attempt", attempt, "maxRetries", MaxSFTPValidationRetries)
		}
	}
	return fmt.Errorf("validation timed out after %d attempts: %w", MaxSFTPValidationRetries, lastErr)
}
